...
Once you are logged in, enter the `adduser your-username` command. This command will start the process of creating a user with a few prompts. The first one will require you to make a password for the user and confirm it. The other prompts may be filled out or left blank.
...
Let’s also make this user have root privileges. To do this we have to edit the `/etc/sudoers` file and add `your-username ALL=(ALL:ALL) ALL` below root as shown below.

Enter `nano /etc/sudoers` to edit the file.
...
Run `ssh-keygen -t rsa` to generate an RSA key pair.
...
`$HOME/.ssh/id_rsa` file: your private key.

`$HOME/.ssh/id_rsa.pub` file: your public key.
...
`ssh-keygen -t rsa -b 4096 -f ~/any/file/you/want -C "your comments here"`
...
We will use the `ssh-copy-id` command to copy your public key to your server.
...
The -i option in the command above gives the file location of your public key. This command will log into your server, take your public key from your local computer and copy it to the `$HOME/.ssh/authorized_keys` file on your server. The command may also request your server user's password to complete the task.
...
If you don\’t have the `ssh-copy-id` command installed, you can pipe the public key over ssh with the following command `cat $HOME/.ssh/id rsa.pub | ssh tjgore@1 04.236.28.27 “cat >> .ssh/authorized_keys”`. It does the same thing as the ssh-copy-id command.
...
If your ssh keys where not generated in the default file location `$HOME/.ssh/id_rsa and $HOME/.ssh/id_rsa.pub`, you should edit your private key file permissions and log in using the following command `ssh -i /public/key/location tj@104.236.28.27`.

Your private key file permissions should be edited because your server will not match up with a private key with incorrect file access permissions. Run `chmod 600 /your-private/key/file-location` to set the correct file permissions for your private key.
